...
import { Search, Filter, Users, ChevronLeft, ChevronRight, Loader2, GraduationCap, X, Lock, Hash } from "lucide-react";
...
const YEARS = Array.from({ length: 20 }, (_, i) => new Date().getFullYear() + 4 - i);
...
function StudentRow({ student }) {
  ...
  return (
    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
        ...
        <Link to={`/profile/${student.id}`} className="flex items-center gap-3 group">
          ...
          <div>
            <p className="text-sm font-medium text-gray-900 group-hover:text-blue-600 transition-colors flex items-center gap-1.5">
              {name}
              {student.is_private && <Lock size={11} className="text-gray-400" title="Private profile"/>}
            </p>
      ...
      <td className="px-5 py-4">
        <p className="text-sm text-gray-700 text-center">{student.batch_year || "—"}</p>
      </td>
    </tr>
  );
}
...
export default function StudentListPage() {
  ...
  async function fetchStudents() {
    ...
    try {
      ...
      const { data } = await api.getStudentList(params);
  ...
  const activeFilters = [program, batchYear].filter(Boolean).length;
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Browse and manage student records.{totalCount > 0 && ` ${totalCount.toLocaleString()} students found.`}
          </p>
        </div>
      </div>
      ...
      {showFilters && (
        <div className="card flex flex-wrap gap-4">
          ...
          <div className="flex-1 min-w-36">
            ...
            <select value={batchYear} onChange={e => { setBatchYear(e.target.value); setPage(1); }} className="input-field bg-white text-sm">
          ...
          {activeFilters > 0 && (
            <div className="flex items-end">
              <button onClick={() => { setProgram(""); setBatchYear(""); setPage(1); }}
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wide">
                    <span className="flex items-center gap-1.5"><Hash size={12}/>Student No.</span>
                  </th>
              ...
              <tbody>
                {students.map(a => <StudentRow key={a.id} student={a}/>)}
              </tbody>
            </table>
          </div>
        )}
